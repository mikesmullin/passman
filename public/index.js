...
function boot() {
	...
	M.mount("#app", () => ({
		...
		vaultName: "My Vault",
		/** Path for new vaults — defaults to the process cwd. User can edit it. */
		...
		defaultCreatePath(name) {
			const filename = this.suggestPathFromName(name);
			const cwd = String(this.cwd || "").trim();
		...
		syncCreatePathDefault() {
			if (this.createPathFollowsName) {
				this.createPath = this.defaultCreatePath(this.vaultName);
			}
		},
		...
		onVaultNameInput(e) {
			...
			this.vaultName = v;
			if (this.createPathFollowsName) {
				this.createPath = this.defaultCreatePath(v);
			}
		},
		...
		async doCreate() {
			...
			try {
				...
				this.meta = res.meta;
				// Keep the newly-created vault selected after locking. The main
				// process persists the normalized path in meta.path.
				this.openPath = res.meta?.path || res.meta?.absolutePath || path;
		...
		async doClose() {
			...
			try {
				...
				if (res.savedPath) this.openPath = res.savedPath;
  ...
  <section class="panel hero wide" x-show="screen === 'create'">
	...
	<label>Vault name
      ...
      <p class="muted" style="font-size:0.8rem;margin:0">
				Defaults to <span class="mono" x-text="cwd || 'the launch directory'"></span>. Relative paths are resolved from there.
      </p>
